...
class Trainner(object):
    def __init__(self, args, model, dataset, criterion, optimizer, logger, d_optimizer=None):
        source_train_loader, source_test_loader, target_train_loader, target_test_loader = dataset
        self.args = args
        self.model = model
    ...
    def train_target_cnnP_domain(self,
        model,
        criterion,
        optimizer,
        d_optimizer,
        ...
        validation = self.validate(model[0], target_test_loader, criterion, args=args)
        ...
        try:
            ...
            for epoch_i in range(1, 1 + args.epochs):
                start_time = time()
                training = self.adversarial_domain(
                    model,
                ...
                validation = self.validate(
                    model[1], target_test_loader, criterion, args=args)
    ...
    def adversarial_domain(self,
        ...
        for iter_i in range(n_iters):
            ...
            D_input_source = model[0].encoder(source_data)
            D_input_target = model[1].encoder(target_data)
            ...
            # train model[2]
            D_output_source = model[2](D_input_source)
            D_output_target = model[2](D_input_target)
            ...
            D_input_target = model[1].encoder(target_data)
            D_output_target = model[2](D_input_target)
            D_output_target_P = model[1].classifier(D_input_target)
    ...
    def step(self, model, data, target, criterion, args):
    ...
    def validate(self, model, dataloader, criterion, args=None):
        model.eval()
        losses = AverageMeter()
        targets, probas = [], []
        ...
        with torch.no_grad():
            for iter_i, (data, target) in enumerate(dataloader):
                bs = target.size(0)
                output, loss = self.step(model, data, target, criterion, args)
    ...
    def train_transformer(self,
        ...
        for epoch_i in range(args.epochs):
            train_loss = self.train_epoch(model, train_loader, criterion, optimizer, args, epoch_i)
            validation = self.validate(model, test_loader, criterion, args=args)
            clsNames = validation['classNames']
            is_best = (best_score is None or validation['avgAcc'] > best_score)
            best_score = validation['avgAcc'] if is_best else best_score
            best_class_score = validation['classAcc'] if is_best else best_class_score
    ...
    def train_epoch(self, model, train_loader, criterion, optimizer, args, epoch_i):
        model.train()
        losses = AverageMeter()
    ...
    def train(self, args):
        if args.model == 'transformer':
            self.train_transformer(self.model, self.source_train_loader, self.source_test_loader, self.criterion, self.optimizer, self.args, self.logger)
        elif args.model == 'adv':
            self.train_target_cnnP_domain(self.model, self.criterion, self.optimizer,
                                          self.source_train_loader, self.target_train_loader, self.target_test_loader, 
